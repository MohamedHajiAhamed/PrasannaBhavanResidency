...
namespace Prasanna_Bhavan_Residency
{
    ...
    {
        public Profile_admin(string user_id, string mail)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                using (SqlConnection connect = new SqlConnection(connectionstring))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        txtbox_password.Text = row["password"].ToString();
                        txtbox_cpassword.Text = row["confirm_password"].ToString();
                        txtbox_mail.Text = row["email"].ToString();
                        txtbox_owner_mail.Text = row["owner_email"].ToString();
                        txtbox_manager_mail.Text = row["manager_email"].ToString();
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string emailAddress = txtbox_mail.Text.Trim();
                    bool isValid = IsEmailValid(emailAddress);
                    string ownerEmail = txtbox_owner_mail.Text.Trim();
                    bool ownermailisValid = IsEmailValid(ownerEmail);
                    string managerEmail = txtbox_manager_mail.Text.Trim();
                    bool managermailisValid = IsEmailValid(managerEmail);
                    if (isValid && ownermailisValid && managermailisValid)
                    {
                        ...
                        if (txtbox_password.Text.Trim().Length >= 8)
                        {
                            if (txtbox_password.Text.Trim() == txtbox_cpassword.Text.Trim())
                            {
                                ...
                                SqlParameter manager_email = new SqlParameter("@manager_email", SqlDbType.VarChar);
                                sp_update_user_single.Parameters.Add(manager_email).Value = txtbox_manager_mail.Text.Trim();
